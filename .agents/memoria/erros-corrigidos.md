...
- **Regra nova:** toda animação cênica decorativa deve ter modo reduce = "lenta e gentil", nunca display:none nem congelada; conteúdo/informação continua sem movimento. Vale para navegadores com reduce, que são mais comuns que parecem (Windows "efeitos de animação" liga reduce global no Chrome).