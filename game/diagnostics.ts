...
/** Frase pronta do tipo "4 de 6 equipes investiram em tecnologia". */
export function diagnosticSentence(entry: DiagnosticEntry): string {
  const verb = entry.teams === 1 ? 'equipe' : 'equipes';
  return `${entry.teams} de ${entry.totalTeams} ${verb}: ${entry.label.toLowerCase()}`;
}
...
export function buildTeachingHooks(
  ...
  if (tech && training && tech.teams > training.teams) {
    hooks.push(
      `${tech.teams} equipes investiram em tecnologia, mas só ${training.teams} buscaram capacitação. Pergunte ao grupo o que aconteceu com o equipamento que ficou parado.`,
    );
  }
  ...
  if (policy && policy.teams < totalTeams) {
    hooks.push(
      `${totalTeams - policy.teams} equipes não buscaram programa público em nenhuma rodada. Pergunte por quê: falta de informação, desconfiança ou pressa?`,
    );
  }
  ...
  if (credit && credit.teams > 0) {
    hooks.push(
      `${credit.teams} equipes recorreram a crédito. Pergunte como a parcela apareceu nas rodadas seguintes e o que isso mudou nas escolhas.`,
    );
  }

  const sustainability = byTag.get('sustainability');
  const production = byTag.get('production_first');
  if (sustainability && production) {
    hooks.push(
      `Produção veio na frente em ${production.teams} equipes e sustentabilidade em ${sustainability.teams}. Pergunte se as duas coisas estavam realmente em conflito ou se pareciam estar.`,
    );
  }
